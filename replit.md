...
New `scoring/amazon_pricer.py` calls Amazon Product Advertising API 5 (`SearchItems`, SDK-free, AWS SigV4 HMAC-SHA256 signed) and returns the same `[{price,title,condition}]` shape as `google_pricer.py` so it slots into the `ebay_pricer.py` retail-price step transparently. Returns `None` when any of the three secrets is missing → caller falls back to the Google scraper. Wrapper at `ebay_pricer.py:1303-1346` tries PA-API first, falls back on `None`/exception/empty, and tags the chosen `_retail_source` (`amazon_paapi`|`google_scraper`|`none`) in one INFO log line per request. `/admin/dashboard` gains an `amazon_paapi` block (`configured`, `last_status`, `last_called_at`, `last_error`, `ok_count`, `error_count`) gated by the existing `X-Admin-Token`. Three secret slots requested but unfilled (`AMAZON_PAAPI_ACCESS_KEY`, `AMAZON_PAAPI_SECRET_KEY`, `AMAZON_PAAPI_PARTNER_TAG`) — Amazon gates PA-API behind 3 qualifying Associate sales in 180d; once issued, drop the values in and live Amazon prices flow on the next request, no redeploy. Tests in `tests/test_amazon_pricer.py` cover missing-creds sentinel, primary success, HTTP-error fallback, throw-then-fallback, both-empty, and malformed-payload tolerance via `httpx.MockTransport`. No version bump (server-only).
